import { accentColor, bgCss, escapeHtml, fgColor, fontFor } from './common.js';
export function compileTypography(p, ctx) {
    ...
    p.lines.forEach((line, j) => {
        ...
        lineCss.push(`#${id}{font-family:'${font}';font-size:${line.size}px;font-weight:${line.weight};color:${color};letter-spacing:${line.tracking}px;line-height:1.15;}`);
        if (p.reveal === 'letter-stagger') {
            ...
            chars.forEach((_, k) => {
                ...
                channels.push({ el: `${id}-c${k}`, p: 'opacity', k: [{ t: d, v: 0 }, { t: d + 320, v: 1, e: 'outCubic' }] }, { el: `${id}-c${k}`, p: 'y', k: [{ t: d, v: 26 }, { t: d + 420, v: 0, e: 'outCubic' }] });
            });
        }
        else if (p.reveal === 'mask-wipe') {
            const inId = `${id}-in`;
            linesHtml.push(`<div class="mask" id="${id}"><span class="mask-in" id="${inId}">${escapeHtml(line.text)}</span></div>`);
            lineCss.push(`#${id}{display:inline-block;overflow:hidden;padding-bottom:0.08em;}#${inId}{display:inline-block;}`);
            channels.push({ el: inId, p: 'yPct', k: [{ t: delay, v: 110 }, { t: delay + p.revealMs, v: 0, e: 'outExpo' }] }, { el: id, p: 'opacity', k: [{ t: delay, v: 0 }, { t: delay + 80, v: 1, e: 'linear' }] });
        }
        else if (p.reveal === 'scale-in') {
            linesHtml.push(`<div class="line" id="${id}">${escapeHtml(line.text)}</div>`);
            channels.push({ el: id, p: 'opacity', k: [{ t: delay, v: 0 }, { t: delay + p.revealMs * 0.5, v: 1, e: 'outCubic' }] }, { el: id, p: 'scale', k: [{ t: delay, v: 0.92 }, { t: delay + p.revealMs, v: 1, e: 'outQuint' }] });
        }
        else {
            linesHtml.push(`<div class="line" id="${id}">${escapeHtml(line.text)}</div>`);
            channels.push({ el: id, p: 'opacity', k: [{ t: delay, v: 0 }, { t: delay + p.revealMs, v: 1, e: 'outCubic' }] }, { el: id, p: 'y', k: [{ t: delay, v: 48 }, { t: delay + p.revealMs, v: 0, e: 'outCubic' }] });
        }
    });
    ...
    const css = [
        `#${sid}{background:${bgCss(p.bg, ctx.tokens)};}`,
