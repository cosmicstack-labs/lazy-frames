import { accentColor, bgCss, escapeHtml, fgColor, fontFor, hexA } from './common.js';
export function compileStatHit(p, ctx) {
    ...
    const vc = `${sid}-vc`;
    const parts = [`<div class="stat-wrap" id="${sid}-wrap">`];
    if (p.kicker)
        parts.push(`<div class="kicker" id="${sid}-k">${escapeHtml(p.kicker)}</div>`);
    parts.push(`<div class="stat-value"><span id="${vc}"></span></div>`);
    if (p.label)
        parts.push(`<div class="stat-label" id="${sid}-l">${escapeHtml(p.label)}</div>`);
    if (p.bars.length > 0) {
    ...
    parts.push('</div>');
    if (p.kicker) {
        channels.push({ el: `${sid}-k`, p: 'opacity', k: [{ t: 0, v: 0 }, { t: 500, v: 1, e: 'outCubic' }] }, { el: `${sid}-k`, p: 'y', k: [{ t: 0, v: 24 }, { t: 500, v: 0, e: 'outCubic' }] });
    }
    channels.push({
        el: vc,
        p: 'textCount',
    ...
    if (p.label) {
        channels.push({ el: `${sid}-l`, p: 'opacity', k: [{ t: 400, v: 0 }, { t: 900, v: 1, e: 'outCubic' }] }, { el: `${sid}-l`, p: 'y', k: [{ t: 400, v: 24 }, { t: 900, v: 0, e: 'outCubic' }] });
    }
    p.bars.forEach((b, i) => {
        channels.push({
            el: `${sid}-b${i}`,
